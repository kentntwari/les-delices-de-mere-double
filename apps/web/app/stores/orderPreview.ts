...
import type { TOrderDTO } from "@repo/shared/mvc/mapper/order";
import type { THandleOrderIntentsSchema } from "@repo/shared/utils/schemas.zod";
...
export const useOrderPreviewStore = defineStore("orderPreview", () => {
  ...
  function commitStatusChange(
    ...
    return requestFetch<void>(`/api/order/${currentPreviewedOrder.value.id}`, {
      method: "PUT",
  ...
  async function fetchRelatedCustomer(): Promise<{
    ...
    try {
      ...
      const t = await requestFetch<{ data: TOrderRelatedCustomer }>(
