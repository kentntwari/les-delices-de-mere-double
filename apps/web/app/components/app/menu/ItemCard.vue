<script lang="ts" setup>
  ...
  async function submitUpdateForm(
    ...
    else if (i === "UPDATE_PRICING")
      emits("update", "UPDATE_PRICING", {
        id: props.item.id,
        unitPrice: v.unitPrice,
      });
    else if (i === "DELETE_ITEM") emits("delete", props.item.id);

    console.log("Submitting edit for item title...", JSON.stringify(v));
    emits("commit", i, {
      ...v,
      title: i !== "UPDATE_TITLE" ? props.item.title : v.title,
      unitPrice: i !== "UPDATE_PRICING" ? props.item.unitPrice : v.unitPrice,
    });
...
<template>
  <UIAlertDialog v-model:open="edit.isDeleting.value">
    <UIContextMenu>
      <UIContextMenuTrigger class="h-fit">
        <article
        ...
        >
          ...
          <footer class="flex flex-col gap-4">
            ...
            >
              <form
                @focusout="edit.revertToDefaultPricingState"
                @submit.prevent
              >
                <Field name="unitPrice" v-slot="{ field }">
    ...
    <UIAlertDialogContent>
      <UIAlertDialogHeader>
        ...
        <UIAlertDialogDescription>
          {{ $t("components.alerts.menu.delete-item.description") }}
        </UIAlertDialogDescription>
      </UIAlertDialogHeader>
      <UIAlertDialogFooter>
        <UIAlertDialogCancel>{{
        ...
        <UIButton
          @click="
            () => {
              ...
              edit.revertToDefaultDeletingState();
            }
          "
