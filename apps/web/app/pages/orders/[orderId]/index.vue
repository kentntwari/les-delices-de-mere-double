<script lang="ts" setup>
  ...
  const commentsDialogOpen = useState<boolean>(
    `order-comments-dialog-open-${orderId.value}`,
    () => false,
  );

  const MAX_COMMENTS_DISPLAYED = 5;
  const hasReachedCommentsLimit = computed(
    () => commentsStore.comments.length >= 10,
  );
  const shouldDisplayCautionMessage = computed(
    () =>
      commentsStore.comments.length > MAX_COMMENTS_DISPLAYED &&
      commentsStore.comments.length < 10,
  );
  const shouldDisplayExcessCommentsTicker = computed(
    () => commentsStore.comments.length > MAX_COMMENTS_DISPLAYED,
  );
  ...
  async function likeComment(commentId: string) {
    await $fetch(`/api/order/${orderId.value}/comments/${commentId}/like`, {
      method: "POST",
      body: {},
    });
  }
  ...
  async function deleteComment(commentId: string) {
    await $fetch(`/api/order/${orderId.value}/comments/${commentId}`, {
      method: "DELETE",
    });
  }
</script>
...
<template>
  ...
  <section class="h-full grid grid-rows-[auto_1fr] mb-10" v-else>
        ...
        <UITabs :default-value="'comments'">
          ...
          <UITabsContent :value="'comments'">
            ...
            >
              ...
              <ul v-show="commentsStore.comments.length > 0" class="space-y-4">
                <li
                  v-for="message in commentsStore.comments.slice(
                    0,
                    MAX_COMMENTS_DISPLAYED,
                  )"
                  :key="message.id"
                  ...
                  <AppOrderChatWrapper :order-id="orderId">
                    <AppOrderChatMessage
                      ...
                      :message="{
                        ...
                        source_comment: message._meta.source,
                        likedCount: message._meta.likedCount,
                        user_name: message._meta.createdBy,
                        taggedUsers: message._meta.mentionedUsers,
                        createdAt: message._meta.createdAt,
                      }"
                      @like="likeComment(message.id)"
                ...
                <li
                  v-if="shouldDisplayExcessCommentsTicker"
                  class="mt-7 font-medium text-primary-400 text-sm"
                >
                  <UIDialog v-model:open="commentsDialogOpen">
                    <LazyUIDialogTrigger class="cursor-pointer">
                      +{{
                        commentsStore.comments.length - MAX_COMMENTS_DISPLAYED
                      }}
                      more comments
                    ...
                    <LazyUIDialogContent
                      class="overflow-hidden lg:overflow-y-hidden"
                      @interact-outside="(e) => e.preventDefault()"
                    >
                      <LazyUIDialogHeader>
                        <LazyUIDialogTitle
                          >{{ $t("pages.order.comments.dialog.header-title") }}
                        </LazyUIDialogTitle>
                        <UIDialogClose />
                      </LazyUIDialogHeader>

                      <VisuallyHidden>
                        <LazyUIDialogDescription />
                      </VisuallyHidden>

                      <ul
                        class="p-6 space-y-2 flex flex-col overflow-auto *:relative"
                      >
                        <li
                          v-for="message in commentsStore.comments"
                          :key="message.id"
                          class="min-w-fit max-w-[600px]"
                        >
                          <LazyAppOrderChatWrapper :order-id="orderId">
                            <LazyAppOrderChatMessage
                              :author="message._meta.createdBy"
                              :message="{
                                id: message.id,
                                comment: message.comment,
                              ...
                              @delete="deleteComment"
                            />
                          </LazyAppOrderChatWrapper>
                        </li>
                      </ul>

                      <LazyUIDialogFooter
                        class="max-h-14"
                        v-if="!hasReachedCommentsLimit"
                      >
                        <LazyAppOrderChatInput class="border-none p-0" />
                      </LazyUIDialogFooter>
                    </LazyUIDialogContent>
                  </UIDialog>
                </li>
              ...
              <ClientOnly>
                <AppOrderChatWrapper
                  ...
                  v-if="
                    !hasReachedCommentsLimit &&
                    !shouldDisplayExcessCommentsTicker
                  "
                >
                  <form @submit.prevent>
                    <AppOrderChatInput
                ...
                <p class="*:text-sm *:text-red-500/70 flex space-x-2" v-else>
                  <LockIcon :size="16" />
                  <small>
                    Comments limit reached. No new comment can be sent</small
                  >
                </p>
              </ClientOnly>
            </div>
    ...
    >
      <div></div>
      <div
        class="h-full bg-slate-200 to-transparent border-b border-neutral-grey-600"
      ></div>
    </div>
  </section>
</template>
